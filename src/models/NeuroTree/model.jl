...
function LuxCore.initialparameters(rng::AbstractRNG, l::NeuroTree)
    return (
        w=Float32.((rand(rng, l.nodes * l.trees, l.feats) .- 0.5) ./ 4),
        b=zeros(Float32, l.nodes * l.trees),
        s=Float32.(fill(log(exp(1) - 1), l.nodes * l.trees)),
        p=Float32.(randn(rng, l.outs, l.leaves * l.trees) .* l.init_scale),
    )
end
...
function (l::NeuroTree)(x, ps, st)
    if l.scaler
        nw = softplus(ps.s) .* (l.actA(ps.w) * x .+ ps.b)
    else
        nw = (l.actA(ps.w) * x .+ ps.b)
    end
    nw = reshape(nw, size(st.ml, 2), :)
    lw = exp.(st.ml * nw .- st.ms * softplus.(nw))
